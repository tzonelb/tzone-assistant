...
- **Bot Triggers (مفعلات البوت)** — the user-defined "Triggers" feature: company-configurable rules that wake the bot on real events. 8 types — event-based (new conversation, appointment booked/completed, task completed, call logged — fired inline from service hooks) and time-based (customer silent N min, team not replying N min, appointment reminder — evaluated every 60s by a background worker). Each firing is deduped (one occurrence never fires twice), notifies the team, and can auto-send a message to the customer on Messenger/Instagram/WhatsApp. `/triggers` page with editor + firing history, `triggers.view`/`triggers.manage` RBAC. 259/259 suite, lint/build clean.
- **Dialer** — real calling behind a provider abstraction (Twilio REST implemented, no SDK): dial pad + customer picker, live-call transfer to an employee's phone, hang-up, call recording, AI auto-answer for inbound calls (company greeting + voicemail + team notification), signature-verified Twilio webhooks, and automatic mirroring of finished calls into the Calls log. `/dialer` page + `dialer.use` permission. **Off until the user provides TWILIO_ACCOUNT_SID / TWILIO_AUTH_TOKEN / TWILIO_PHONE_NUMBER / PUBLIC_BASE_URL** — the page shows a setup notice and refuses to dial (clean 503, never a crash) until then. WhatsApp-call routing note: WhatsApp has no public API for answering/routing its voice calls; the dialer calls the customer's phone number from the same profile the WhatsApp conversation links to. Full conversational voice-AI (streaming STT/TTS) is a documented upgrade path. 266/266 suite, lint/build clean.
...
| Feature | State | Notes |
|---|---|---|
| **Dialer LIVE activation** | needs user credentials | The code is fully built and tested (see DONE). To go live: create a Twilio account, buy a number, set `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_PHONE_NUMBER`, `PUBLIC_BASE_URL` in `.env`, and point the Twilio number's Voice webhook at `POST {PUBLIC_BASE_URL}/api/dialer/webhooks/inbound`. |
...
| **Conversational voice AI** | future upgrade | The AI currently answers inbound calls with a spoken greeting + voicemail recording. A full talking AI (real-time STT/TTS over Twilio Media Streams) is a separate, larger build. |

**Every feature the user requested is now built.** All sidebar entries lead to real, working, permission-gated, company-scoped pages.
