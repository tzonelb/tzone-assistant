...
import { isUiV2Enabled, setUiV2Enabled } from "../../config/featureFlags";
    ...
    <section className="user-settings-shell">
      ...
      <main className="user-settings-content">
        <div className="user-settings-content-scroll">
          ...
          {active === "appearance" ? <section className="settings-section-card"><h3>New interface</h3><p>The redesigned navigation and layout — on by default while the remaining screens are still being rebuilt one by one. Turn it off to use the classic layout in this browser only.</p><div className="settings-notification-row"><div><strong>Use the new interface</strong><span>Applies immediately in this browser only. Nobody else's session is affected.</span></div><Toggle checked={uiV2} onChange={(value) => { setUiV2(value); setUiV2Enabled(value); }} /></div></section> : null}
