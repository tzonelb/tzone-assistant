    ...
    <div className="workflow-wizard-panel">
      {current.key === "greeting" ? (
        <div className="workflow-setting-row" style={{ flexDirection: "column", alignItems: "stretch", gap: 8 }}>
          {/* Disabled, and pointing at the real one, because nothing reads
              `company_profile.greeting_message`: the greeting the assistant
              actually sends is the bot profile's `welcome_message_ar` /
              `welcome_message_en`, gated by `welcome_enabled`, and Test & Train
              AI is where they are edited. Wiring this box to those is not a
              rename -- the design draws one field where the platform keeps two,
              one per language, so which one a single box writes is a product
              decision. Saying so beats a box that saves and changes nothing. */}
          <div><strong>Greeting message</strong><span>Edited under Test &amp; Train AI, where the assistant keeps a separate welcome for Arabic and for English.</span></div>
          <textarea rows={3} value="" disabled placeholder="Set the welcome message under Test &amp; Train AI" style={{ padding: "8px 10px", borderRadius: 8, border: "1px solid #d5dae5" }} readOnly />
        </div>
      ) : null}
      {current.key === "who_may" ? (
...
function ProfileSettings() {
  ...
  return (
    <div className="workflow-settings-card">
      ...
      {/* The design drew an editable "Workspace code" here. It is not one on
          this platform, twice over: sign-in stopped asking for a workspace
          code (tests/test_the_login_form_is_not_a_user_directory.py), and the
          real code is the second wrapping of the company's database key --
          `platform_service` hands it back once at creation and never stores it
          readable. An input bound to `company_profile.workspace_code` writes a
          string into an open settings bag that nothing reads, while telling
          the owner they changed the code that protects their data. Removed
          rather than disabled: there is no value to show in it. */}
      <div className="workflow-setting-row">
