...
def meta_client(monkeypatch):
    ...
    monkeypatch.setattr(config, "META_APP_SECRET", TEST_APP_SECRET)
...
def whatsapp_client(monkeypatch):
    ...
    # This branch's whatsapp webhook hands messages to
    # process_whatsapp_message (not message_gateway.handle_text) -- the
    # signature gate runs BEFORE it, which is exactly what these tests
    # prove (rejected requests never reach it).
    def fake_process_whatsapp_message(**kwargs):
        calls.append(kwargs)
        return {"company_id": 1, "queue_result": {"queued": True}}

    monkeypatch.setattr(
        whatsapp_webhook_module,
        "process_whatsapp_message",
        fake_process_whatsapp_message,
    )
    # Make the test payload's phone_number_id resolve via the legacy
    # single-number path so no channel_accounts DB setup is needed --
    # these tests are about the signature gate, not tenant routing.
    monkeypatch.setattr(config, "WHATSAPP_PHONE_NUMBER_ID", "1234567890")
