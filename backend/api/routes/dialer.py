...
# RBAC notes: viewing dialer state/history uses the Calls module's
# "calls.view"; actually placing/transferring/hanging up calls requires
# the dedicated "dialer.use" permission (both seeded in database.py and
# auto-granted to the owner role like every other code).
def _require(
    current_user: dict[str, Any], company_id: int, permission_code: str
) -> None:
    ...
    if not allowed:
        raise HTTPException(
            ...
            detail="You do not have dialer access.",
        )
...
def dialer_status(context=Depends(current_context)):
    ...
    _require(current_user, company_id, "calls.view")
...
def place_call(payload: PlaceCallRequest, context=Depends(current_context)):
    current_user, company_id = context
    _require(current_user, company_id, "dialer.use")
